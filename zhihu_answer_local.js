...
const answers = [
  {
    questionUrl: 'https://www.zhihu.com/question/280696372',
...
技术门槛比SEO低，但时间窗口很短。`,
  ...
  {
    questionUrl: 'https://www.zhihu.com/question/386174803',
    content: `GEO是Generative Engine Optimization的缩写，中文叫"生成式引擎优化"。
...
3. 监测品牌在AI搜索中的表现`,
  ...
  {
    questionUrl: 'https://www.zhihu.com/question/434461306',
...
时间窗口大概还剩6-12个月，现在开始正合适。`,
  ...
  {
    questionUrl: 'https://www.zhihu.com/question/374249438',
...
两者不是替代关系，而是互补。SEO做基础流量，GEO做增量流量。`,
  ...
  {
    questionUrl: 'https://www.zhihu.com/question/638464105',
...
**③ 内容策略变了**
SEO需要大量长尾关键词文章（100篇起）。GEO需要少量高质量的权威内容（10篇精品就够）。
...
对于中小跨境电商卖家来说，GEO的性价比远高于传统SEO。`,
  ...
  {
    questionUrl: 'https://www.zhihu.com/question/268595834',
...
这套流程我已经跑了3天，第一篇文章上线后24小时就被AI索引了。`,
  ...
  {
    questionUrl: 'https://www.zhihu.com/question/408175213',
...
这周目标是完成8篇知乎回答，每天在群里分享进展。`,
  ...
  {
    questionUrl: 'https://www.zhihu.com/question/260104887',
...
(async () => {
  ...
  try {
    ...
    // 检查是否已登录
    const loginCheck = await page.$('.AppHeader-profileAvatar');
    if (!loginCheck) {
      console.log('⚠️ 未检测到登录状态，请在打开的浏览器中手动登录知乎');
      console.log('   登录后按回车继续...');
      await new Promise(resolve => process.stdin.once('data', resolve));
    }
    
    console.log('✅ 已登录知乎');
    console.log(`\n📝 准备发布 ${answers.length} 篇回答...\n`);
    
    for (let i = 0; i < answers.length; i++) {
      const a = answers[i];
      console.log(`\n--- 第 ${i+1}/${answers.length} 篇 ---`);
      console.log(`🔗 ${a.questionUrl}`);
      
      try {
        await page.goto(a.questionUrl, { waitUntil: 'networkidle', timeout: 30000 });
        ...
        // 填入内容
        const editor = await page.$('.DraftEditor-root');
        if (editor) {
          await editor.click();
          await page.waitForTimeout(500);
          await page.keyboard.type(a.content, { delay: 30 });
          await page.waitForTimeout(1000);
          console.log('✅ 内容已填入');
          ...
          console.log('⚠️ 未找到编辑器，可能问题已关闭回答');
        }
      } catch (err) {
    ...
    console.log('\n🎉 全部完成！');
    console.log('   请检查知乎账号的回答列表确认发布成功');
